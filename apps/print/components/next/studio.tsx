...
import { alignLayer, ARTWORK_FONTS, artworkFingerprint, artworkProduct, artworkSvg, copyDesign, designForFace, printFaces, downloadArtwork, export300DpiPng, imagePercentSize, imagePpi, preflightArtwork, rasterDimensions, unrotatedLayerBounds } from '../../lib/next/artwork';
...
type Props = { design: Design; onChange: (design: Design) => void; onSave: (design: Design) => Promise<void>; onQuote: (design: Design) => void; brand: BrandKit; saving: boolean; onExit?: () => void; storageStatus?: 'loading' | 'saving' | 'saved' | 'error' | 'conflict'; onStorage?: () => void; onReturn?: () => void; returnLabel?: string; savingreturn?: boolean };
...
export default function PrintStudio({ design: project, onChange, onSave, onQuote, brand, saving, onExit, storageStatus, onStorage, onReturn, returnLabel = 'Update estimate artwork', savingreturn = false }: Props) {
  ...
  const undo = () => { const previous = history.current.past.pop(); if (!previous) return; history.current.future.push({ design: projectCurrent.current, face: previous.face }); restoreHistory(previous); };
  const redo = () => { const next = history.current.future.pop(); if (!next) return; history.current.past.push({ design: projectCurrent.current, face: next.face }); restoreHistory(next); };
  ...
  return <section className="studio-app" aria-label="Artwork studio">
    <header className="studio-heading">
      <div className="studio-document">
        <button className="studio-back" onClick={onExit} disabled={savingreturn || !onExit} aria-label={onReturn ? 'Back to estimate' : 'All projects'} title={onReturn ? 'Back to estimate' : 'All projects'}><ArrowLeft size={19}/><span>{savingreturn ? 'Saving…' : onReturn ? 'Back to estimate' : 'All projects'}</span></button>
      ...
      <div className="studio-heading-actions">
        ...
        <button className="studio-button studio-save" aria-label="Save project" onClick={() => void save()} disabled={saving || !design.name.trim()}><Save size={15}/><span>{saving ? 'Saving…' : 'Save project'}</span></button>
        <button className="studio-button studio-primary" disabled={savingreturn} onClick={onReturn || (() => setProofOpen(true))}><ShieldCheck size={15}/><span>{savingreturn ? 'Updating…' : onReturn ? returnLabel : 'Review artwork'}</span></button>
      </div>
    </header>
        ...
        <div className="studio-inspector-scroll">
          {sideTab === 'design' ? <>
            ...
            <div className="studio-property-section studio-preflight">
              ...
              <button className="studio-button studio-full studio-review-action" onClick={() => setProofOpen(true)}><ShieldCheck size={15}/>Review artwork</button>
    ...
    {proofOpen && <div className="studio-modal-backdrop" onPointerDown={event => { if (event.target === event.currentTarget) setProofOpen(false); }}>
      <div ref={proofDialog} className="studio-proof" role="dialog" aria-modal="true" aria-labelledby="studio-proof-title">
        ...
        <div className="studio-proof-body">
          ...
          <div className="studio-proof-details"><h3>{design.name}</h3><p>{p.name} · {p.width} × {p.height} mm<br/>{design.quantity.toLocaleString()} items · {design.finish} · {design.tier}<br/>{faces.length === 2 ? 'Front and back' : 'Front only'}</p>
