...
/** Formats with two independently editable, equal-size print faces. */
export function supportsReverse(p: Product): boolean { return ['cards','postcard','flyer','letterhead','invitation','menu'].includes(p.id); }
...
/** A non-destructive rectangle in the original image, normalized to 0–1. */
export type ImageCrop={x:number;y:number;width:number;height:number};
export type DesignLayer={id:string;type:'text'|'image'|'shape';text:string;x:number;y:number;size:number;color:string;rotation:number;opacity:number;src?:string;assetPath?:string;width?:number;height?:number;font?:string;weight?:number;naturalWidth?:number;naturalHeight?:number;crop?:ImageCrop};
export type DesignFace={background:string;layers:DesignLayer[]};
export type Design={id:string;name:string;productId:string;background:string;layers:DesignLayer[];back?:DesignFace;quantity:number;tier:Tier;finish:string;sides:number;city:string;imageWidth?:number;imageHeight?:number;version:number;updatedAt:string};
