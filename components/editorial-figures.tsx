...
export function ServicesVisual() {
  return (
    <section
      className="editorial-visual services-visual"
      aria-labelledby="services-system-title"
    >
      <div className="editorial-image-panel">
        ...
        <div className="editorial-image-copy">
          ...
          <p>
            Specialists enter through different doors. The brief, decision trail
            and measure of success stay shared.
          </p>
        </div>
      </div>
      <figure className="service-system-figure">
        <figcaption>
          ...
          <p>
            Every discipline orbits the same business objective—so the campaign,
            website, media and measurement do not contradict one another.
          </p>
        </figcaption>
        <div
          className="service-orbit"
          aria-label="Six connected agency disciplines surrounding one business objective"
        >
...
export function SolutionsVisual() {
  ...
  return (
    <section
      className="editorial-visual solutions-visual"
      aria-labelledby="solutions-route-title"
    >
      ...
      <figure className="outcome-route-figure">
        <figcaption>
          <span>THE RESULT ROUTE</span>
          <p>
            A clear commercial result determines the strategy, the creative
            system and the evidence required.
          </p>
        </figcaption>
        <ol>
  ...
  return (
    <figure className="about-atlantic-visual">
      ...
      <figcaption>
        ...
        <p>
          Built with a Newfoundland point of view. Designed to work across
          Canada.
        </p>
      </figcaption>
    </figure>
...
export function InvestmentFigure() {
  ...
  return (
    <figure className="investment-figure">
      <figcaption>
        <span>THE INVESTMENT ARCHITECTURE</span>
        <h2>Every dollar has a job.</h2>
        <p>
          A proposal should separate the cost of thinking, making, reaching,
          operating and learning—then show how they work together.
        </p>
      </figcaption>
      <ol>
...
export function TrustMap({ eyebrow, title, items }: TrustMapProps) {
  return (
    ...
    >
      <header>
        <span>{eyebrow}</span>
        <h2 id={`${eyebrow.toLowerCase().replaceAll(" ", "-")}-title`}>
          {title}
        </h2>
      </header>
      <div>
        {items.map(([name, description, icon]) => {
          ...
          return (
            <article key={name}>
              ...
              <b>{name}</b>
              <p>{description}</p>
              <CircleCheckBig aria-hidden="true" className="trust-map-check" />
            </article>
          );
        })}
      </div>
  ...
  return (
    <figure className="platform-signal-figure">
      <figcaption>
        ...
        <h2>Brief → decision → delivery → result.</h2>
      </figcaption>
      <div
        className="platform-signal-line"
        aria-label="Connected client workflow"
      >
        {["Brief", "Decision", "Production", "Launch", "Learning"].map(
          (item, index) => (
            <div key={item}>
