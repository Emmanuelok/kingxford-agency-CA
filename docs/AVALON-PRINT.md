...
### Connected customer journey acceptance — 12 September 2026

The public deployment was reviewed after comparing Printful, Printify and VistaPrint's official workflows. A keyboard product search opened the new configurator. Selecting 250 business cards, Soft touch and Front and back produced the expected $34.43 CAD planning subtotal, and those settings persisted into a named template project. Separate reverse-side text survived undo/redo and appeared alongside the front in artwork review.

Creating an estimate retained both faces and specifications. Adding a mug returned to that same estimate, producing two items and a $49.38 CAD known subtotal. Reopening the card item, changing its back and updating estimate artwork retained exactly two items and left the mug unchanged. Review displayed all three print faces. Package download completed through the visible controls on desktop and phone; the quotation email draft contained the correct product brief and recipient. No email was sent. Archive integrity, per-face SVG contents, CRC values and escaping were covered by the package tests.

Responsive review covered the storefront, configuration, estimate details and review package in a 390 px frame, with no page-level horizontal overflow. The two-face phone editor measured 390 × 800 px with matching scroll dimensions, and Back to estimate returned successfully. The review layout also fit a 768 px frame without horizontal overflow. Visual review led to consistent forest-green estimate styling, larger instructional copy, singular item/object labels and an explicit mobile Estimates accessible name. The temporary review helper was removed.

Focused recovery checks also exercised the actual estimate-return callbacks: failed-save add and replacement retries, draft-only return, mismatched draft routes and stale estimate revisions all preserved the intended data. Full build, print tests, agency tests, lint and HTTP smoke passed in GitHub CI for the customer journey release; subsequent presentation changes received targeted lint and print build checks.
