...
export default function PricingPage(){return <main className="site-shell"><SiteNav/><PageHero index="07" eyebrow="WAYS TO WORK" title="Clear ranges." accent="Considered investment." description="These are Canadian-dollar planning ranges—not a quote. They help us begin with commercial fit, then scope the real work transparently."/><section className="pricing-note content-section"><div className="section-kicker"><span>CAD</span> PLANNING GUIDANCE</div><h2>Every part of the investment, clearly scoped.</h2><p>Final estimates separate strategy, creative, production, talent, travel, permits, printing, licensing, technology, media and taxes. You should always know what each dollar is for.</p></section><InvestmentFigure/><section className="pricing-grid content-section">{offers.map((o,i)=><article key={String(o[0])}><span>0{i+1}</span><h2>{String(o[0])}</h2><b>{String(o[1])}</b><p>{String(o[2])}</p><ul>{(o[3] as string[]).map(x=><li key={x}><Check/>{x}</li>)}</ul><a href="/start">Build this brief <ArrowUpRight/></a></article>)}</section><section className="next-cta acid-block"><span>Smaller budget or uncertain scope?</span><h2>Start with diagnosis.</h2><a className="button button-dark" href={WORKSPACE_ENABLED ? "/tools" : "/start"}>{WORKSPACE_ENABLED ? "Use the free planning tools" : "Discuss your priorities"} <ArrowUpRight/></a></section><SiteFooter/></main>}
